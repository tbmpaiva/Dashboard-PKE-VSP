      ...
      {payload.map((entry: any) => (
        <div key={entry.name} className="flex items-center justify-between gap-4 py-0.5">
          ...
          <span className="mono font-semibold">
            {entry.name === "Tx Conv" ? `${entry.value}%` : entry.value.toLocaleString("pt-PT")}
          </span>
        </div>
      ))}
    </div>
...
export default function EvolutionChart({ data, isWeekly }: Props) {
      ...
      <div className="p-4">
        <ResponsiveContainer width="100%" height={280}>
          <ComposedChart data={data} margin={{ top: 4, right: 4, bottom: 4, left: 0 }}>
            ...
            <XAxis
              dataKey="period"
              tick={{ fill: "#7A9CC4", fontSize: 11, fontFamily: "JetBrains Mono" }}
              axisLine={false}
              tickLine={false}
            />
            <YAxis
            ...
            <Legend
              wrapperStyle={{ paddingTop: 12, fontSize: 11, color: "#7A9CC4" }}
            />
            <Bar
              yAxisId="left"
              dataKey="trafego"
